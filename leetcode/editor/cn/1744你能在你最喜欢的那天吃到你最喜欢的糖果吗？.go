...
func canEat(candiesCount []int, queries [][]int) []bool {
	rexNum := make([]int, len(candiesCount)+1) //前缀和数组
	for i := 1; i <= len(candiesCount); i++ {
		rexNum[i] = rexNum[i-1] + candiesCount[i-1] //计算前缀和数组
	}
	eatMin := 1 //最小吃的个数
	answer := make([]bool, len(queries))
	for i, v := range queries {
		candyType := v[0]  //要吃的糖果类型
		eatDay := v[1] + 1 //要吃的天数
		eatMax := v[2]     //规定最大吃的个数
		if eatDay*eatMax > rexNum[candyType] && eatDay*eatMin <= rexNum[candyType+1] {
			answer[i] = true
		}
	}
	return answer
