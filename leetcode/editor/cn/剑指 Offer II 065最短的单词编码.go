...
type PrefixTree struct {
	next   map[byte]*PrefixTree
	length int
	isCnt  bool
}

func minimumLengthEncoding(words []string) (ret int) {
	preT := &PrefixTree{next: make(map[byte]*PrefixTree)}
	for _, s := range words {
		root := preT
		cnt := 0
		for i := len(s) - 1; i >= 0; i-- {
			c := s[i]
			if root.next[c] == nil {
				root.next[c] = &PrefixTree{next: make(map[byte]*PrefixTree)}
			}
			root = root.next[c]
			cnt++
		}
