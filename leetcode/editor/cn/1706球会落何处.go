...
// grid[i][j] 为 1 或 -1
//
// Related Topics 深度优先搜索 数组 动态规划 矩阵 模拟 👍 87 👎 0
...
func findBall(grid [][]int) (ret []int) {
	for i := range grid[0] { //遍历每个球
		x, y := 0, i //球的初始下标
		for {
			if x >= len(grid) { //终止条件，掉出去了
				ret = append(ret, y)
				break
			}
			if ny := y + grid[x][y]; ny < 0 || ny >= len(grid[0]) || grid[x][y] != grid[x][ny] { //看能不能去往要去的地方
				ret = append(ret, -1)
				break
			}
			x, y = x+1, y+grid[x][y] //更新下标
		}
	}
	return
}
