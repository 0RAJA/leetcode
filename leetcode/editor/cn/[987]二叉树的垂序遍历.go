...
func verticalTraversal(root *TreeNode) (ret [][]int) {
	...
	dfs = func(root *TreeNode, row, col int) {
		...
		value[col] = append(value[col], node)
		dfs(root.Left, row+1, col-1)
		dfs(root.Right, row+1, col+1)
	}
	dfs(root, 0, 0)
	for i := min; i <= max; i++ {
		sort.Slice(value[i], func(x, y int) bool {
			if value[i][x].row != value[i][y].row {
				return value[i][x].row < value[i][y].row
			} else {
				return value[i][x].val < value[i][y].val
			}
		})
		var x []int
