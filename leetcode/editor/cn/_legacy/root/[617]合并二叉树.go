 ...
 * }
 */
// 合并二叉树：合并当前节点，返回合并后节点
func mergeTrees(root1 *TreeNode, root2 *TreeNode) *TreeNode {
	if root1 == nil && root2 == nil {
		// 当前两个节点都是 nil，返回 nil
		return nil
	} else if root1 != nil && root2 != nil {
		// 当前两个节点都不为空，则合并当前节点，并对左右子树分别递归合并
		root1.Val, root2.Val = root1.Val+root2.Val, root1.Val+root2.Val
		root1.Left = mergeTrees(root1.Left, root2.Left)
		root1.Right = mergeTrees(root1.Right, root2.Right)
		return root1
	} else if root1 != nil {
		// 只有 root1 不为 nil，直接返回 root1
		return root1
	} else {
		// 只有 root2 不为 nil，直接返回 root2
		return root2
	}
}
