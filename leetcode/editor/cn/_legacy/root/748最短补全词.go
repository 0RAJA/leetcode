...
func shortestCompletingWord(licensePlate string, words []string) (ret string) {
	cntMap := [26]int{} //记录出现的字符数
	num := 0            //记录需要消去的字符个数
	for i := range licensePlate {
		if c := rune(licensePlate[i]); unicode.IsLetter(c) {
			cntMap[unicode.ToLower(c)-'a']++
			num++
		}
	}
	for i := range words {
		maps := map[rune]int{} //用于cntMap恢复字符个数
		...
		if num == 0 && (len(words[i]) < len(ret) || ret == "") {
			ret = words[i]
		}
		for c, v := range maps { //恢复数量
			if cntMap[c] <= 0 {
				num += cntMap[c]
			}
			num += v
