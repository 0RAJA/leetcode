...
//
// Related Topics 深度优先搜索 图 拓扑排序 数组 👍 187 👎 0
...
func loudAndRich(richer [][]int, quiet []int) (ret []int) {
	link := make([][]int, len(quiet)) // 富->穷... 连接关系
	inCnt := make([]int, len(quiet))  // 富 数 入度
	for _, r := range richer {
		link[r[0]] = append(link[r[0]], r[1])
		inCnt[r[1]]++
	}
	ret = make([]int, len(quiet))
	for i := range ret { //默认都是自己
		ret[i] = i
	}
	q := list.New()
	for i, cnt := range inCnt { //先将所有入度为0的加入队列(最富有的)
		if cnt == 0 {
			q.PushBack(i)
		}
	}
	for q.Len() > 0 {
		idx := q.Front().Value.(int)
		q.Remove(q.Front())
		for _, v := range link[idx] { //遍历连接关系,并更新结果.
			if quiet[ret[v]] > quiet[ret[idx]] {
				ret[v] = ret[idx]
			}
			inCnt[v]--
			if inCnt[v] == 0 {
