...
// parents 表示一棵二叉树。
//
// Related Topics 树 深度优先搜索 数组 二叉树 👍 66 👎 0
...
func countHighestScoreNodes(parents []int) int {
	nums := make([]int, len(parents))     //nums[i] = i节点的下挂节点数(包括自己)
	childs := make([][]int, len(parents)) //记录节点对应孩子的下标
	for i, pre := range parents {
		if pre >= 0 {
			childs[pre] = append(childs[pre], i)
		}
	}
	var maxValue, maxCnt int
	var dfs func(idx int) int
	dfs = func(idx int) (ret int) { //计算下挂节点数
		defer func() { nums[idx] = ret }()
		for _, c := range childs[idx] {
			ret += dfs(c)
		}
		return ret + 1
	}
	...
	for i, pre := range parents {
		sum := 1
		if pre >= 0 {
			sum *= nums[0] - nums[i] //父子树的节点数
		}
		for _, c := range childs[i] {
			sum *= nums[c] //左和右孩子树的节点数
		}
		if sum == maxValue {
			maxCnt++
