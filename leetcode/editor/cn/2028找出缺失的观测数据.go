...
func missingRolls(rolls []int, mean int, n int) (ret []int) {
	sum := 0
	for _, v := range rolls {
		sum += v
	}
	if sum+n > mean*(len(rolls)+n) || sum+6*n < mean*(len(rolls)+n) {
		return []int{}
	}
	p := mean*(len(rolls)+n) - sum
	ret = make([]int, n)
	for i := range ret {
		x := p / n
		ret[i] = x
