...
/*
	dp[i][j][k] 前i个公司，j个人，最小价值为k的情况下的方案数。
	dp[0][0][0] = 1
		dp[i][j][k] = dp[i-1][j][k]
		if j >= cost(可以工作) :
			dp[i][j][k] += dp[i-1][j-cost][max(0,j-earn)] (因为超过的也可以算进去) 有点像选n个数凑目标值
*/
func profitableSchemes(n int, minProfit int, group []int, profit []int) (ret int) {
	...
	dp := make([][][]int, len(group)+1) //工作,人数,钱
	for i := range dp {
		dp[i] = make([][]int, n+1)
		for j := range dp[i] {
	...
	for i, v := range group {
		...
		for j := 0; j <= n; j++ {
			for k := 0; k <= minProfit; k++ {
				dp[i+1][j][k] = dp[i][j][k]
				if j >= v {
					dp[i+1][j][k] = (dp[i+1][j][k] + dp[i][j-v][max(k-e, 0)]) % MOD
				}
			}
		}
	}
	for _, p := range dp[len(group)] {
		ret = (ret + p[minProfit]) % MOD
	}
	return
}
