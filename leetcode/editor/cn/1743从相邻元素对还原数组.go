...
func restoreArray(adjacentPairs [][]int) (ret []int) {
	...
	for i := 0; i < len(adjacentPairs); i++ {
		mapCnt[adjacentPairs[i][0]]++
		mapCnt[adjacentPairs[i][1]]++
		mapValue[adjacentPairs[i][0]] = append(mapValue[adjacentPairs[i][0]], adjacentPairs[i][1])
		mapValue[adjacentPairs[i][1]] = append(mapValue[adjacentPairs[i][1]], adjacentPairs[i][0])
	}
	var head, tail int
	flag := false
	...
	for {
		...
		ret[cnt] = head
		cnt++
		value := mapValue[head][0]
		if mapOK[value] == true {
			value = mapValue[head][1]
		}
		mapOK[head] = true
		head = value
	}
